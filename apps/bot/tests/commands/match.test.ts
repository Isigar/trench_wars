...
//
// The vi.mock for ../../src/services/api replaces the module-level `api`
// object with a spy. The mock is hoisted by Vitest before the
// import-under-test resolves, so the command module sees the spies, not the
// real network client.
...
describe('/match list subcommand', () => {
    ...
    it('calls api.get(/matches) with actsAsDiscordId set to interaction.user.id', async () => {
        vi.mocked(api.get).mockResolvedValue({ data: [] });
        const interaction = makeInteraction('list');
        await execute(interaction as unknown as ChatInputCommandInteraction);
