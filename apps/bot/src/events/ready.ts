...
// amended by 05-11-PLAN.md task 1 (Wave 10) to launch the outbound poll
// loop on gateway-ready. Centralises the boot-time work that depends on
// the gateway being live:
//
//   1. Log the bot tag (operator confirmation Discord auth worked).
//   2. Register slash commands to the league's guild (Pattern 3).
//   3. Start the outbound polling worker (plan 05-11 Pattern 4).
//
...
// reconnects). The outbound worker is also started ONLY once per process
// — the module-level `running` flag in outbound.ts would be unsafe if
// multiple intervals were attached to the same Client.
//
...
import { startOutboundWorker } from '../services/outbound.js';
...
export function registerReadyHandler(client: Client): void {
    client.once(Events.ClientReady, (c) => {
        ...
        // Pattern 4 outbound poll loop — picks up pending
        // discord_outbound_messages rows every env.OUTBOUND_POLL_INTERVAL_MS
        // (default 5000) and dispatches them to Discord.
